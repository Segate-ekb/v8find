...
Процедура ОбновитьТаблицуВерсий()
	...
	Если ТаблицаВерсийПлатформы.Количество() = 0 Тогда
		Лог.Предупреждение("Не найдено ни одной установленной версии платформы 1С (каталогов поиска: %1)",
			МассивКаталоговПоиска.Количество());
	КонецЕсли;
...
Процедура СобратьСтандартныеПутиУстановки()
	...
	Если ЭтоWindows Тогда
		...
		Если Это64ОС Тогда
			...
			Если Не ЗначениеЗаполнено(КаталогПрограммWindows_x64) Тогда
				// Дочерние процессы могут получать урезанное окружение без ProgramW6432,
				// при этом ProgramFiles в нем обычно присутствует
			...
			Если ЗначениеЗаполнено(КаталогПрограммWindows_x64) Тогда
			...
			КонецЕсли;
			Если ЗначениеЗаполнено(КаталогПрограммWindows_x86) Тогда
		...
		Иначе
			...
			Если ЗначениеЗаполнено(КаталогПрограммWindows) Тогда
				ДобавитьКаталоГПоиска(ПутьУстановки_x86_1Cv8, РазрядностьПлатформы.x86, Неопределено);
				ДобавитьКаталоГПоиска(ПутьУстановки_x86_1Cv82, РазрядностьПлатформы.x86, Неопределено);
			КонецЕсли;
...
Функция СобратьВозможныеКаталогиУстановкиПлатформыWindows()
	...
	Если Не ЗначениеЗаполнено(КаталогВсеПользователи) Тогда
		КаталогВсеПользователи = ПолучитьПеременнуюСреды("ProgramData");
	КонецЕсли;
	...
	ДополнитьМассивРасположенийИзКонфигурационногоФайла(КаталогВсеПользователи, СуффиксРасположения, МассивПутей);
	ДополнитьМассивРасположенийИзКонфигурационногоФайла(КаталогТекущегоПользователя, СуффиксРасположения, МассивПутей);
...
Процедура ДополнитьМассивРасположенийИзКонфигурационногоФайла(Знач БазовыйКаталог, Знач СуффиксРасположения, Знач МассивПутей)

	Если Не ЗначениеЗаполнено(БазовыйКаталог) Тогда
		Возврат;
	КонецЕсли;

	ИмяФайла = ОбъединитьПути(БазовыйКаталог, СуффиксРасположения);
